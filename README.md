...
面向教师和数学内容创作者的本地 AI 数学动画工作台。它把数学教学 Prompt 转换为可审阅、可渲染的 Manim 动画，当前支持公式推导和函数可视化。

核心工作流：

    教学 Prompt → ContentPlan → CodeVersion → Preview/Final 视频 → 质量诊断与产物下载

生成的 Python 代码不会直接执行，必须通过静态安全检查、结构检查和版本持久化，再由 Runner 在隔离环境中渲染。

## 当前能力

- 账号登录、首次登录强制修改密码、会话 Cookie 和 CSRF 防护
- 项目、Prompt、ContentPlan、CodeVersion 的不可变版本链
- 公式推导和函数可视化两种生成类别
- 完整 Manim Community 代码生成，代码在工作台中只读展示
...
| tests/phase*、tests/web | 分阶段 Python、边界和 Web 测试 |
...
按提示输入两次初始密码。密码至少 14 位；首次登录后系统会强制修改密码。
...
如果 Windows 到 WSL 的 localhost 转发正常：
    ...
    cd /home/<user>/projects/Manim_project/apps/web
    NEXT_PUBLIC_API_URL=http://localhost:8000 ../../node_modules/.bin/next dev --hostname 0.0.0.0
...
1. 打开登录页并登录。首次登录先设置新密码。
2. 创建项目，例如“二次函数顶点公式推导”。
3. 在“创作输入”填写教学 Prompt、受众、时长、推导风格和明确假设。
...
只有浏览器侧的 NEXT_PUBLIC_API_URL 使用 WSL IP。
...
4. API 健康检查返回 200；
5. 浏览器地址、NEXT_PUBLIC_API_URL 和 MANIM_WORKBENCH_ALLOWED_ORIGINS 使用同一个 WSL IP。
