...
- [ ] 定义 6 个场景、12 次运行及统一结果格式。
- [ ] 以测试固定淘汰、权重和 10 分以内优先 CE 的选择规则。
- [ ] 提交可独立验证的评分器。

### Slice 2: 并行引擎基准

- [ ] Terra-CE 实现并运行 ManimCE 基准。
- [ ] Terra-GL 实现并运行 ManimGL 基准。
- [ ] 每个实现记录精确依赖、命令、日志、耗时和失败原因。

### Slice 3: 父 agent 核验与选型

- [ ] 检查场景语义对等、12 次运行真实性和输出文件证据。
- [ ] 运行统一评分器并完成人工视觉评分。
- [ ] 输出选型报告，锁定引擎、FFmpeg、LaTeX 和字体版本。
- [ ] 更新项目计划、README 和任务清单。
