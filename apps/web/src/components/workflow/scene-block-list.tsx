  ...
  return (
    <section aria-labelledby="scene-list-title">
      ...
      <div className={styles.sceneList}>
        {model.scenes.map((scene, index) => <SceneBlockCard key={scene.localId} index={index} count={model.scenes.length} scene={scene} previewRun={model.runFor(scene, "preview")} finalRun={model.runFor(scene, "final")} onUpdate={(patch) => model.updateScene(scene.localId, patch)} onGenerate={(profile) => void model.generateScene(scene, profile)} onUploadCsv={(csvText) => void model.uploadCsv(scene, csvText)} onMove={(delta) => model.moveScene(index, delta)} onCopy={() => model.copyScene(scene)} onRemove={() => model.removeDraft(scene.localId)} onDragStart={() => setDragged(index)} onDrop={() => { if (dragged !== null) model.reorderScene(dragged, index); setDragged(null); }} />)}
      </div>
    </section>
  );
}
